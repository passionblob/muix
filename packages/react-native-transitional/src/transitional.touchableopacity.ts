...
import { Animated, TouchableOpacityProps, ViewStyle, TouchableOpacity } from 'react-native'
...
export class TransitionalTouchableOpacity extends Component<TouchableOpacityProps & { config?: TransitionConfig }> {
  private anim = new Animated.Value(1)
  private styleHolder: StyleHolderOf<TouchableOpacityProps> = {
    style: createStyleHolder(),
  }

  private progress = 0
  constructor(props: Readonly<TouchableOpacityProps & { config?: TransitionConfig }>) {
    super(props)
  ...
  render(): React.ReactNode {
    ...
    const transitionalStyles = getTransitionalStyles<TouchableOpacityProps>({
      anim: this.anim,
      interpolator,
...
export default TransitionalTouchableOpacity
